...
var jsonFiles = ["boys.json", "men.json", "women.json", "girls.json"];
var accessoriesData = [];

jsonFiles.forEach(function (jsonFile) {
  var xhr = new XMLHttpRequest();
  xhr.open("GET", "https://www.ramees.dev/product_data/" + jsonFile, true);
  ...
  xhr.onload = function () {
    ...
    if (status === 200) {
      ...
      var filteredData = data.filter(function (item) {
        return item.masterCategory === "Accessories";
      });
      accessoriesData = accessoriesData.concat(filteredData);
      if (jsonFile === "girls.json") {
